...
func (m *kubeGenericRuntimeManager) updateContainerResources(pod *v1.Pod, container *v1.Container, containerID kubecontainer.ContainerID) error {
	...
	//TODO: Windows support has been supported. We need more intelligent way here.
...
func toKubeContainerStatus(status *runtimeapi.ContainerStatus, runtimeName string) *kubecontainer.Status {
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.InPlacePodUpdate) {
		...
		// TODO: how to know if the resources is from Windows or Linux?
	...
	cStatus := &kubecontainer.Status{
		...
		Resources:    v1.ResourceRequirements{Requests: resourceRequests, Limits: resourceLimits},
	}
