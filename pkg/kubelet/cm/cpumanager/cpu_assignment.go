...
type sortCPUsPackedOrSpreadFuncs interface {
	sortAvailableCores() []int
	allocateFullCores()
}
...
var _ sortCPUsPackedOrSpreadFuncs = (*sortCPUsPacked)(nil)
var _ sortCPUsPackedOrSpreadFuncs = (*sortCPUsSpread)(nil)

func (s sortCPUsPacked) sortAvailableCores() []int {
	return s.acc.sortAvailableCPUsPacked()
}

func (s sortCPUsPacked) allocateFullCores() {
	// When using sortCPUsPacked, we proceed to check and claim entire cores.
	for _, core := range s.acc.freeCores() {
		cpusInCore := s.acc.topo.CPUDetails.CPUsInCores(core)
		if !s.acc.needsAtLeast(cpusInCore.Size()) {
			continue
		}
		klog.V(4).InfoS("takeFullCores: claiming core", "core", core)
		s.acc.take(cpusInCore)
	}
}

func (s sortCPUsSpread) sortAvailableCores() []int {
	return s.acc.sortAvailableCPUsSpread()
}

func (s sortCPUsSpread) allocateFullCores() {
	// When using sortCPUsSpread, we allocate individual CPUs rather than entire cores.
...
// when the user specifies the `SpreadPCPUsPreferredOption` static policy option.
...
type cpuAccumulator struct {
	...
	// sortCPUsPackedOrSpread is used to control the cpu sorting result.
	...
	sortCPUsPackedOrSpread sortCPUsPackedOrSpreadFuncs
}
...
func newCPUAccumulator(topo *topology.CPUTopology, availableCPUs cpuset.CPUSet, numCPUs int, cpuSortingStrategy CPUSortingStrategy) *cpuAccumulator {
	...
	if cpuSortingStrategy == CPUSortingStrategyPacked {
		acc.sortCPUsPackedOrSpread = &sortCPUsPacked{acc}
	} else {
		acc.sortCPUsPackedOrSpread = &sortCPUsSpread{acc}
	}
...
// Returns free CPU IDs as a slice sorted by sortCPUsPackedOrSpread().
func (a *cpuAccumulator) freeCPUs() []int {
	return a.sortCPUsPackedOrSpread.sortAvailableCores()
}
...
func (a *cpuAccumulator) takeFullCores() {
	// Delegates to sortCPUsPackedOrSpread to decide on taking full cores based on the CPU sorting strategy.
	a.sortCPUsPackedOrSpread.allocateFullCores()
}

func (a *cpuAccumulator) takeRemainingCPUs() {
	for _, cpu := range a.sortCPUsPackedOrSpread.sortAvailableCores() {
